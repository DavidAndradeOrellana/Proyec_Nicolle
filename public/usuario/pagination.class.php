...
class PerPage {
	...
	function perpage($count,$href) {
		...
		if($pages>1) {
			if(($_GET["page"]-3)>0) {
				if($_GET["page"] == 1)
					$output = $output . '<span IdUsuario=1 class="current-page">1</span>';
				else				
			...
			for($i=($_GET["page"]-2); $i<=($_GET["page"]+2); $i++)	{
				...
				if($i>$pages) break;
				if($_GET["page"] == $i)
					$output = $output . '<span IdUsuario='.$i.' class="current-page">'.$i.'</span>';
				else				
			...
			if(($pages-($_GET["page"]+2))>0) {
				if($_GET["page"] == $pages)
					$output = $output . '<span IdUsuario=' . ($pages) .' class="current">' . ($pages) .'</span>';
				else				
